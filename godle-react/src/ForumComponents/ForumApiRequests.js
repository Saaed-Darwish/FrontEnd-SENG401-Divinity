export const fetchThreads = async (email) => {
    try {
        const response = await fetch(`http://divinity-java-backend-production.up.railway.app/Forum?email=${email}`);
        if (!response.ok) {
...
export const postThread = async (email, title, body) => {
    ...
    try {
        fetch('http://divinity-java-backend-production.up.railway.app/Forum', {
            method: 'POST',
            headers: {
